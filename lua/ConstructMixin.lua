...
function ConstructMixin:OnConstructUpdate(deltaTime)
	...
	local nearbymultiplier = #GetEntitiesForTeamWithinRange("Player",self:GetTeamNumber(), Vector(self:GetOrigin()), 6)
	
    // Only Alien structures auto build.
    // Update build fraction every tick to be smooth.
    if not self:GetIsBuilt() or (not GetIsAlienUnit(self) and not self:isa("CommandStructure") or not self:isa("ResourceTower"))   then
        ...
        if not self.GetCanAutoBuild or self:GetCanAutoBuild() then
            ...
            multiplier = multiplier * ( (HasMixin(self, "Catalyst") and self:GetIsCatalysted()) and kNutrientMistAutobuildMultiplier or 1 ) * (1 + nearbymultiplier)
