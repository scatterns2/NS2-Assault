...
function GUIProgressBar:Update(deltaTime)
    ...
    for index, techPoint in ipairs(GetEntitiesWithinRange("TechPoint", player:GetOrigin(), 5)) do
		...
		local team = player:GetTeamType()
	
		if team == kAlienTeamType then 
			objectiveFraction = techPoint.aliencaptureProgress 
		elseif team == kMarineTeamType then 
			objectiveFraction = techPoint.marinecaptureProgress 
		end	
	end
    for index, resPoint in ipairs(GetEntitiesWithinRange("ResourcePoint", player:GetOrigin(), 10)) do

		local team = player:GetTeamType()
	
		if team == kAlienTeamType then 
			objectiveFraction = resPoint.aliencaptureProgress 
		elseif team == kMarineTeamType then 
			objectiveFraction = resPoint.marinecaptureProgress 
		end	
	end
