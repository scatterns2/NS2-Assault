...
function Hive:OnConstructionComplete()
	...
	local right = self:GetCoords().xAxis
	local forward = self:GetCoords().zAxis
					
	//CreateEntity( Veil.kMapName, self:GetOrigin() + Vector(0, -2, -2), kAlienTeamType)						
	//CreateEntity( Shell.kMapName, self:GetOrigin() + Vector(0, -2, 2), kAlienTeamType)							
	//CreateEntity( Spur.kMapName, self:GetOrigin() + Vector(0, -2, 3), kAlienTeamType)			
    self.bioMassLevel = 1
