...
function NS2Gamerules_GetUpgradedDamageScalar( attacker )
    ...
    if GetHasTech(attacker, kTechId.ThreeCommandStations, true) then            
        return kWeapons3DamageScalar                
    elseif GetHasTech(attacker, kTechId.TwoCommandStations, true) then            
        return kWeapons2DamageScalar                
    elseif GetHasTech(attacker, kTechId.CommandStation, true) then            
...
kDamageType = enum( {'Normal', 'Light', 'Heavy', 'Puncture', 'Structural', 'StructuralHeavy', 'Splash', 'Gas', 'NerveGas',
           'StructuresOnly', 'Falling', 'Door', 'Flame', 'Infestation', 'Corrode', 'ArmorOnly', 'Biological', 'StructuresOnlyLight', 'Spreading', 'GrenadeLauncher' } )
