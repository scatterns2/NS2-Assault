...
function PlayingTeam:GetHasTeamLost()
	...
	if GetGamerules():GetGameStarted() and not Shared.GetCheatsEnabled() then
			...
			local chairAlive = #GetEntitiesForTeam("MainCommandStation", self:GetTeamNumber())
			local cragHiveAlive = #GetEntitiesForTeam("CragHive", self:GetTeamNumber())
		   
			if kMarineTeamType == self:GetTeamType() then
			...
			elseif kAlienTeamType == self:GetTeamType() then
				if  (not activePlayers and not abilityToRespawn)  or 
					self:GetHasConceded() or
					(cragHiveAlive == 0) then
